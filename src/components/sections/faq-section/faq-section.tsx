...
const FaqSection = () => {
  return (
    <section id="faq" className="max-w-2xl w-full mx-auto overflow-x-hidden min-w-0">
      <div className="container mx-auto px-4 py-16 max-w-2xl w-full overflow-x-hidden min-w-0">
        ...
        <div className="w-full max-w-2xl mx-auto min-w-0 overflow-x-hidden">
          <Accordion type="single" collapsible className="w-full max-w-2xl mx-auto min-w-0">
            {FAQS.map((faq, index) => (
              <AccordionItem key={index} value={`item-${index}`}>
                <AccordionTrigger className="text-left break-words">
                  {faq.question}
                </AccordionTrigger>
                <AccordionContent className="break-words">{faq.answer}</AccordionContent>
              </AccordionItem>
            ))}
          </Accordion>
        </div>
